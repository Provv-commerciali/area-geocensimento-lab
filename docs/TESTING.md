...
Vitest covers Zod validators, civic generation/parity/deduplication, filter semantics including zero-interview status, structured floors, controlled vocabularies, persistence outcomes, appraisal invariants, record helpers and repository contracts. React Testing Library covers cascade behavior, conditional UI, absence of interview fields from contact creation and visible database failures. Playwright smoke specifications cover login/demo entry, zone/street flow, contact creation/filtering, contextual street contacts and complex interiors.
