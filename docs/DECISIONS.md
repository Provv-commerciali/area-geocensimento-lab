...
- **DEC-025** — The verified AdE `CP.CadastralParcel` WMS layer is accessed through a parameter-allowlisting same-origin proxy because the upstream omits CORS. OpenLayers requests EPSG:4258 and reprojects to its web view. WFS is not used because the probed endpoint denied access.
