...
describe("census domain rules", () => {
  ...
  it("links multiple subjects to the same property",()=>{const record={...records[0],subjectLinks:[{subjectId:subjects[0].id,role:"Proprietario" as const,isPrimary:true},{subjectId:subjects[1].id,role:"Inquilino" as const,isPrimary:false}]};expect(subjectsForRecord(record,subjects).map(subject=>subject.id)).toEqual([subjects[0].id,subjects[1].id])});
});
