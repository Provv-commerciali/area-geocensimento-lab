...
const repository=readFileSync("src/repositories/supabase-census-repository.ts","utf8");
describe("dashboard event attribution migration",()=>{
  ...
  it("keeps the responsible operator embed unambiguous after adding actor relationships",()=>{expect(repository).toContain("responsible_operator:operators!census_records_responsible_operator_id_fkey");expect(repository).toContain("object(r.responsible_operator)")});
});
