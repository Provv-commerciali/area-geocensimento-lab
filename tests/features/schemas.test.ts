...
describe("runtime validators", () => {
  ...
  it("accepts only controlled engagement types",()=>{expect(censusRecordSchema.safeParse({...base,engagementType:"Incarico altre agenzie",engagementExpiresOn:"2026-12-31"}).success).toBe(true);expect(censusRecordSchema.safeParse({...base,engagementType:"Da definire"}).success).toBe(false)});
  it("requires expiry only for external and exclusive assignments",()=>{expect(censusRecordSchema.safeParse({...base,engagementType:"In esclusiva"}).success).toBe(false);expect(censusRecordSchema.safeParse({...base,engagementType:"In esclusiva",engagementExpiresOn:"2026-12-31"}).success).toBe(true);expect(censusRecordSchema.safeParse({...base,engagementType:"Verbale",engagementExpiresOn:"2026-12-31"}).success).toBe(false)});
  ...
  it("validates interview dates and controlled responses", () => {expect(interviewSchema.safeParse({operatorId:"op",interviewDate:"2026-09-11",response:"Risposto"}).success).toBe(true);expect(interviewSchema.safeParse({operatorId:"op",interviewDate:"2026-09-11",response:"Positiva"}).success).toBe(false)});
