...
export async function loadComplexPhotoData(complexId: string): Promise<ComplexPhotoData> {
  ...
  const proposals = rows.parse(proposalData).map<DoorbellProposal>((proposal) => ({
    ...
    status: proposal.status === "CREATED" ? "CREATED" : proposal.status === "DISCARDED" ? "DISCARDED" : "DRAFT", existingSubjectId: text(proposal.existing_subject_id), civicId: text(proposal.address_access_id),
