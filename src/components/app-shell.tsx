...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? "is-open" : ""}`}>
      ...
      <nav>{items.map(({ href, label, icon: Icon }) => <Link key={href} href={href} prefetch={false} onClick={() => setOpen(false)} className={path === href ? "active" : ""}><Icon size={19}/><span>{label}</span></Link>)}</nav>
