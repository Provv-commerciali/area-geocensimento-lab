...
export const records: CensusRecord[] = Array.from({ length: 36 }, (_, index) => {
  ...
  const createdAt=`2026-0${(index % 8) + 1}-12`;const responsible=operators[index%operators.length];
  const engagementType=(["Nessuno", "Incarico altre agenzie", "In esclusiva", "Verbale", "Non esclusivo"] as const)[index % 5];
  const engagementExpiresOn=engagementType==="Incarico altre agenzie"||engagementType==="In esclusiva"?`2026-${String(9+(index%3)).padStart(2,"0")}-${String(18+(index%8)).padStart(2,"0")}`:undefined;
  return {
    ...
    inherited: index % 11 === 0, responsibleOperatorId: responsible.id,
    responsibleOperatorName: responsible.name,createdByOperatorId:responsible.id,createdByOperatorName:responsible.name,
    ...(isNews?{newsFoundAt:createdAt,newsFoundByOperatorId:responsible.id,newsFoundByOperatorName:responsible.name}:{}),zoneId: zone.id, zoneName: zone.name,
