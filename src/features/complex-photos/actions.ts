...
const proposalSchema = z.object({ proposalId: id, complexId: id, subjectType: z.enum(["PRIVATO","AZIENDA","UNKNOWN"]), firstName: optionalText, lastName: optionalText, companyName: optionalText, existingSubjectId: id.optional().or(z.literal("")), civicId: id.optional().or(z.literal("")), buildingScope: z.enum(["Intero edificio","Parte di edificio"]), staircase: optionalText, unitIdentifier: optionalText, floorCode: z.enum(floorCodes).optional().or(z.literal("")), totalFloors: z.number().int().positive().optional(), qualification: z.enum(qualifications).optional().or(z.literal("")), occupancy: z.enum(occupancies).optional().or(z.literal("")), contactType: z.enum(contactTypes), responsibleOperatorId: id.optional().or(z.literal("")), status: z.enum(["DRAFT","DISCARDED"]) }).superRefine((value, context) => { if (value.subjectType === "PRIVATO" && !value.lastName) context.addIssue({ code: "custom", path: ["lastName"], message: "Cognome obbligatorio" }); if (value.subjectType === "AZIENDA" && !value.companyName) context.addIssue({ code: "custom", path: ["companyName"], message: "Ragione sociale obbligatoria" }); if (value.buildingScope === "Parte di edificio" && !value.floorCode) context.addIssue({ code: "custom", path: ["floorCode"], message: "Piano obbligatorio" }); });
export async function saveDoorbellProposalAction(input: z.input<typeof proposalSchema>) {
  const value = proposalSchema.parse(input); const { db, user } = await authenticatedClient();
  ...
  const { error } = await db.from("doorbell_contact_proposals").update(payload).eq("id", value.proposalId); if (error) throw new Error(error.message); refresh(value.complexId);
}
