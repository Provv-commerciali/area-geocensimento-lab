...
import { CENSUS_OPERATIONAL_STATUS_VISUALS, deriveCensusOperationalStatus, operationalStatusLabel, type CensusOperationalSettings } from "@/domain/census-operational-status";
...
type Props = { records: CensusRecord[]; civics: Civic[]; zones: CensusZone[]; streets: Street[]; complexes: Complex[]; operators: Operator[]; settings: CensusOperationalSettings; today: string; initialFilters: GeoCensusFilters; paidAuthorized:boolean };
type GeocodingResponse = { results?: Array<{ label: string; longitude: number; latitude: number }>; error?: string };

export function GeoCensusMap({ records, civics, zones, streets, complexes, operators, settings, today, initialFilters, paidAuthorized }: Props) {
  ...
  useEffect(() => {
    ...
    map.on("singleclick", async (event) => {
      ...
      if (hit) {
        ...
        if (children.length > 1 && (map.getView().getZoom() ?? 0) < 17) { map.getView().fit(boundingExtent(children.map((child) => (child.getGeometry() as Point).getCoordinates())), { padding: [80, 80, 80, 80], maxZoom: 17, duration: 250 }); }
        setSelected(payloads); setCadastralInfo(null);
      }
      const url = parcelInfo.getFeatureInfoUrl(event.coordinate, map.getView().getResolution()!, "EPSG:3857", { INFO_FORMAT: "text/html" });
      if (!url) return; if(!hit)setSelected([]); setCadastralInfo("loading");
  ...
  const cadastralZone=cadastralInfo&&typeof cadastralInfo==="object"?resolveCadastralTerritory(zones,cadastralInfo.municipalityCode):undefined;const selectedPhoto=selected.flatMap(feature=>feature.records).find(record=>record.photoUrl)?.photoUrl;
  ...
  return <div className="geocensus-layout">
    ...
    <section className="map-workspace">
      ...
      <div className="map-legend"><strong>Legenda</strong>{Object.entries(CENSUS_OPERATIONAL_STATUS_VISUALS).map(([status, visual]) => <span key={status}><i style={{ background: visual.markerColor }}/>{visual.label}</span>)}<span><i className="automatic-ring"/> Posizione da verificare</span><span><i className="complex-ring"/> Complesso</span></div>{selectedPhoto&&<img className="map-contact-photo" src={selectedPhoto} alt="Foto del contatto censito"/>}
      {(selected.length > 0 || cadastralInfo) && <aside className="map-detail">{selected.map(feature=><article className="building-detail-card" key={feature.civicId}><header><span>Edificio / civico</span><strong>{feature.address}</strong><small>{feature.records.length} unità censite nel marker</small></header><div className="building-unit-list">{feature.records.map(record=>{const status=deriveCensusOperationalStatus({contactType:record.contactType,interviews:record.interviews,staleNewsDays:settings.staleNewsDays,today});return <div key={record.id}><strong>{record.unitIdentifier?`Interno ${record.unitIdentifier}`:record.subaltern?`Sub. ${record.subaltern}`:"Unità censita"} · {record.lastName} {record.firstName}</strong><small>{record.floorLabel??"Piano non indicato"} · {operationalStatusLabel(status)}</small><Link href={`/censimento/contatti/${record.id}`}>Apri contatto</Link></div>})}</div></article>)}{cadastralInfo && <article className="cadastral-detail-card"><header><span>Catasto ufficiale</span><strong>Dati catastali dell’edificio</strong></header>{typeof cadastralInfo === "object" ? <><dl><div className="territory-field"><dt>Provincia</dt><dd>{cadastralZone?.province ? `${cadastralZone.province}${cadastralZone.provinceCode?` (${cadastralZone.provinceCode})`:""}` : "Non determinabile"}</dd></div><div className="territory-field"><dt>Comune</dt><dd>{cadastralZone?.municipality??cadastralInfo.municipalityName??"Non determinabile"}</dd></div><div><dt>Codice catastale</dt><dd>{cadastralInfo.municipalityCode}</dd></div><div><dt>Sezione</dt><dd>{cadastralInfo.section ?? "Non restituita"}</dd></div><div><dt>Foglio</dt><dd>{cadastralInfo.sheet}</dd></div><div><dt>Particella</dt><dd>{cadastralInfo.parcel}</dd></div><div><dt>Tipologia immobile</dt><dd>{cadastralInfo.featureType ?? "Non restituita"}</dd></div></dl>{cadastralZone?.provinceCode&&<CadastralServicesPanel compact authorized={paidAuthorized} censusedRecords={selected.flatMap(feature=>feature.records)} context={{provinceCode:cadastralZone.provinceCode,municipality:cadastralZone.municipality,municipalityCode:cadastralInfo.municipalityCode,section:cadastralInfo.section,sheet:cadastralInfo.sheet,parcel:cadastralInfo.parcel}}/>}</> : <p>{cadastralInfo === "loading" ? "Interrogazione particella…" : cadastralInfo === "empty" ? "Nessuna particella disponibile in questo punto." : "Servizio catastale temporaneamente non disponibile."}</p>}<footer>Il click usa solo la cartografia gratuita. I servizi a consumo partono esclusivamente dopo conferma.</footer></article>}</aside>}
    </section>
  </div>;
}
