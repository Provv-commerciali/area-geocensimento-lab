...
export function CadastralPicker({recordId,civic,address,databaseMode,onClose,onSaved}:Props){
  ...
  async function confirm(){if(!feature)return;setSaving(true);const result=await confirmCadastralAssociationAction({recordId,...feature,...(clickPoint?{sourceReference:{longitude:clickPoint[0],latitude:clickPoint[1]}}:{})});setSaving(false);if(!result.ok){setMessage(result.error);return}onSaved(feature);onClose()}
  return <div className="picker-backdrop" role="dialog" aria-modal="true" aria-label="Individua particella sulla mappa"><section className="picker-card"><header><div><strong><Layers size={18}/> Individua particella sulla mappa</strong><span>{address} · cartografia catastale attiva</span></div><button type="button" className="icon-button" aria-label="Annulla selezione particella" onClick={onClose}><X/></button></header><p className="picker-message">{message}</p><div ref={targetRef} className="picker-map" aria-label="Mappa per associare la particella"/>{feature&&<div className="cadastral-preview"><strong>Anteprima — dati effettivamente restituiti</strong><dl><div><dt>Comune catastale</dt><dd>{feature.municipalityName??"non restituito"} ({feature.municipalityCode})</dd></div><div><dt>Sezione</dt><dd>{feature.section??"non restituita"}</dd></div><div><dt>Foglio</dt><dd>{feature.sheet}</dd></div><div><dt>Particella</dt><dd>{feature.parcel}</dd></div><div><dt>Tipo</dt><dd>{feature.featureType??"non restituito"}</dd></div></dl><small>Subalterno, categoria, titolarità e visure non sono restituiti e non vengono creati.</small></div>}<footer><button type="button" className="button secondary" onClick={onClose}>Annulla</button><button type="button" className="button primary" disabled={!feature||saving||!databaseMode} onClick={()=>void confirm()}>{saving?"Salvataggio…":"Conferma associazione"}</button></footer></section></div>}
