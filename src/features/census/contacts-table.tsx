...
export function ContactsTable({ records, zones, streets, complexes, operators, operationalSettings, operationalToday, fixedZoneId, fixedStreetId }: Props) {
  ...
  const update = (key: keyof CensusFilters, value: string) => setFilters((current) => ({ ...current, [key]: value || undefined }));
  ...
  return <>
    ...
    <section className="table-panel">
      ...
      <div className="table-scroll"><table><thead><tr><th>Contatto</th><th>Tipologia</th><th>Stato operativo</th><th>Zona / Indirizzo</th><th>Piano</th><th>Giorni dall'ultimo contatto</th><th>Ricontatto</th><th>Complesso</th><th>Ereditato</th><th>Azioni</th></tr></thead><tbody>{visible.map((record) => {
        ...
        return <tr key={record.id}><td><strong>{record.lastName} {record.firstName}</strong><small>{record.phone ?? "—"}</small></td><td><span className={`badge ${record.contactType === "Notizia" ? "news" : ""}`}>{record.contactType}</span>{record.isAppraised && <small>Periziato</small>}</td><td><OperationalStatusBadge result={status}/></td><td><strong>{record.zoneName}</strong><small>{record.streetName}, {record.civicNumber}{record.civicExtension ? `/${record.civicExtension}` : ""}</small></td><td>{record.floorLabel ?? "—"}</td><td><strong>{status.daysSinceLastInterview === null ? "Mai contattato" : `${status.daysSinceLastInterview} gg`}</strong><small>{status.lastInterviewAt ?? "Nessuna intervista"}</small></td><td>{status.isRecallOverdue ? `Scaduto da ${status.overdueRecallDays} gg` : pendingRecall ?? "—"}</td><td>{record.complexName ?? "—"}</td><td>{record.inherited ? "Sì" : "No"}</td><td><Link className="row-action" href={`/censimento/contatti/${record.id}`}>Apri</Link></td></tr>;
      })}</tbody></table></div>
    </section>
  </>;
}
