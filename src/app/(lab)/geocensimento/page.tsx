...
export default async function GeoCensimentoPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [params, data, paidAuthorized] = await Promise.all([searchParams, loadCensusData(["records", "zones", "streets", "civics", "complexes", "operators", "operationalSettings"]),getCadastralAccess()]);
