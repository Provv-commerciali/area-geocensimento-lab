...
export default async function RecordPage({params,searchParams}:{params:Promise<{recordId:string}>;searchParams:Promise<{interviewCreated?:string;updated?:string}>}){
  ...
  return <><PageHeader eyebrow="Scheda contatto" title={contactName} description={`${record.zoneName} · ${record.streetName}, ${record.civicNumber}${record.civicExtension?`/${record.civicExtension}`:""}`} action={<div className="button-row"><Link className="button secondary" href={`/censimento/contatti/${record.id}/modifica`}>Modifica contatto</Link><OperationalStatusBadge result={operationalStatus}/></div>}/>{interviewCreated==="1"&&<div className="success-banner">Intervista registrata nello storico di questo contatto.</div>}{updated==="1"&&<div className="success-banner">Contatto aggiornato correttamente.</div>}
