...
export default async function StreetPage({params}:{params:Promise<{zoneId:string;streetId:string}>}){
  ...
  return <><PageHeader eyebrow="Zona / Via o indirizzo" title={street.name} description={[street.localityName,zone.municipality].filter(Boolean).join(" · ")} action={<div className="button-row"><Link className="button secondary" href={`/censimento/zone/${zone.id}`}>Torna alla zona</Link><Link className="button primary" href={`/geocensimento?zone=${zone.id}&street=${street.id}`}>Visualizza in GeoCensimento</Link></div>}/><section className="zone-kpis" aria-label="Riepilogo via"><div><strong>{street.totalAccesses}</strong><span>Civici</span></div><div><strong>{records.length}</strong><span>Contatti</span></div></section><section className="panel"><h2>Civici</h2><p className="muted">Consulta i civici disponibili e i relativi contatti.</p><StreetCivicsList accesses={accesses} contactCounts={contactCounts} contactRecordIds={contactRecordIds}/>{street.totalAccesses>accesses.length&&<p className="muted">Affina la ricerca per consultare altri civici.</p>}</section></>;
}
